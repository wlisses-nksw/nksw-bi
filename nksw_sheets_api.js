...
var CONFIG = {
  ...
  ABA_RANK:        'Base Produto Cor',  // ranking de mais vendidos por produto/cor
...
function getClientes(params) {
  try {
    ...
    // R: percentil de recencia (menor = mais recente = score maior)
    var sortedRec = clientList.map(function(c) { return c.recency; }).sort(function(a, b) { return a - b; });
    var rp20 = sortedRec[Math.floor(sn * 0.2)] || 0;
    var rp40 = sortedRec[Math.floor(sn * 0.4)] || 0;
    var rp60 = sortedRec[Math.floor(sn * 0.6)] || 0;
    var rp80 = sortedRec[Math.floor(sn * 0.8)] || 0;
...
function getRankData(ss) {
  try {
    ...
    return { rankData: rankData, mesesKeys: mesesKeys };
  } catch(e) {
    return { rankData: [], mesesKeys: [], error: e.message };
  }
}
