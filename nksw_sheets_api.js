...
/* ===== CLIENTES — RFM + Cohort a partir da Base Completa ===== */
function getClientes(params) {
  params = params || {};
  var cohortMonths = parseInt(params.cohortMonths) || 6; // meses de follow-up além do Mês 0
  var rows = readSheet(CONFIG.ABA_VENDAS, CONFIG.ID_VENDAS);
  if (rows.length < 2) return { kpis: {}, rfm: [], cohort: [] };
  ...
  var iData      = findCol(h, cm.data);
  var iReceita   = findCol(h, cm.venda_sem_frete); // usa venda sem frete como valor de referência
  var iRecFallbk = findCol(h, cm.receita);         // fallback se sem frete não existir
  var iStatusPag = findCol(h, cm.status_pag);
  var iNome      = findCol(h, cm.nome);

  if (iEmail < 0) {
    return { kpis: {}, rfm: [], cohort: [], erro: 'Coluna de e-mail não encontrada. Cabeçalhos: ' + h.join(', ') };
  }

  var iValFinal = iReceita >= 0 ? iReceita : iRecFallbk;
  var today     = new Date(); today.setHours(0, 0, 0, 0);

  // ── Agrupa pedidos por e-mail ─────────────────────────────────────────
  var clientes = {};
  ...
  rows.slice(1).forEach(function(r) {
    ...
    // Ignora cancelados/estornados
    if (iStatusPag >= 0) {
      var st = trim(r[iStatusPag]).toLowerCase();
    ...
    var val  = iValFinal >= 0 ? parseNum(r[iValFinal]) : 0;
    var nome = iNome >= 0 ? trim(r[iNome]) : '';

    if (!clientes[email]) clientes[email] = { nome: nome, orders: [] };
    clientes[email].orders.push({ data: d, receita: val });
  });

  var clientList = Object.keys(clientes).map(function(e) {
    return { email: e, nome: clientes[e].nome, orders: clientes[e].orders };
  });

  if (!clientList.length) return { kpis: {}, rfm: [], cohort: [] };
  ...
  clientList.forEach(function(c) {
    ...
    if (c.numOrders > 1) {
      ...
      for (var i = 1; i < c.orders.length; i++) {
        gap += (c.orders[i].data - c.orders[i-1].data) / 86400000;
      }
      c.avgGap = gap / (c.numOrders - 1);
    }
  });
  ...
  // ── RFM Scoring ───────────────────────────────────────────────────────
  var sorted = clientList.map(function(c) { return c.totalSpent; }).sort(function(a,b){return a-b;});
  var n = sorted.length;
  var p20 = sorted[Math.floor(n * 0.2)] || 0;
  var p40 = sorted[Math.floor(n * 0.4)] || 0;
  var p60 = sorted[Math.floor(n * 0.6)] || 0;
  var p80 = sorted[Math.floor(n * 0.8)] || 0;

  function rScore(d) { return d<=30?5:d<=60?4:d<=90?3:d<=180?2:1; }
  function fScore(n) { return n>=9?5:n>=5?4:n>=3?3:n>=2?2:1; }
  function mScore(v) { return v>p80?5:v>p60?4:v>p40?3:v>p20?2:1; }
  ...
  var rfm = Object.keys(rfmDefs)
    ...
    .map(function(k) {
      ...
      return {
        ...
        base:     g.base,
        pedidos:  g.pedidos,
        receita:  round2(g.receita),
        acao:     rfmDefs[k].acao,
      };
    })
    .sort(function(a, b) { return b.receita - a.receita; });

  // ── Cohort Analysis — últimas 10 safras ──────────────────────────────
  ...
  var cohort = Object.keys(cohortMap).sort().slice(-13).map(function(key) {
    ...
    return {
      safra:    MESES_PT[g.mo] + '. ' + g.yr,
      clientes: cn,
      receita:  round2(totalRec),
      cac:      0,
      gasto:    0,
      m:        m,
    };
  ...
  return {
    kpis: {
      total:          total,
      recompras:      recompras,
      pctRecomp:      total > 0 ? round1(recompras / total * 100) : 0,
      ltv:            round2(ltv),
